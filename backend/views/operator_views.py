    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Methods_Emails.send_info_email(
                request, model.operator_username, model.operator_name, 'Your password has been reset successfully.')
            messages.info(
...
class AjaxOperatorsList(View):
    ...
    def _datatables(self, request, operator):
        ...
        column2 = 'operator_name'
        column3 = 'operator_contact_phone_number'
        column4 = 'operator_organization_id'
        ...
        if operator.operator_organization_id != 0:
            objects.filter(
                Q(operator_organization_id=operator.operator_organization_id))
        ...
        if search:
            ...
            objects = objects.filter(
                ...
                Q(operator_contact_phone_number__icontains=search) |
        ...
        if column_search != '':
            objects_filter = True
            objects = objects.filter(
                Q(operator_name__icontains=column_search)
            )
        ...
        if column_search != '':
            objects_filter = True
            objects = objects.filter(
                Q(operator_contact_phone_number__icontains=column_search)
            )
        ...
        for record in object_list:
            ...
            value2 = OperatorsTable.render_operator_name(record)
            value3 = OperatorsTable.render_operator_contact_phone_number(
                record)
            ...
            data.append({
                ...
                'operator_name': value2,
                'operator_contact_phone_number': value3,
                'operator_organization_id': value4,
...
def select_single(request):
    ...
    try:
        ...
        if operator.operator_organization_id != 0 and operator.operator_organization_id != model.operator_organization_id:
            return HttpResponseForbidden('Forbidden', content_type='text/plain')
    except(TypeError, ValueError, OverflowError, Operators.DoesNotExist):
        return HttpResponseBadRequest('Bad Request', content_type='text/plain')
...
def update_reset_password(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            model.save()
            Methods_Emails.send_info_email(
                request, model.operator_username, model.operator_name, 'Your password has been reset successfully by admin.')
