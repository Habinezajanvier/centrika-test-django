...
class Utils(object):
    ...
    def get_current_datetime_utc():
        ...
        return datetime.datetime.now(tz=timezone.utc).strftime(settings.APP_CONSTANT_INPUT_DATETIME_FORMAT)

    @staticmethod
    def get_midnight_datetime_utc():
        now = datetime.datetime.utcnow()
        if settings.TIME_IN_SECONDS:
            return round(now.replace(hour=0, minute=0, second=0, microsecond=0).timestamp())
        return now.replace(hour=0, minute=0, second=0, microsecond=0)

    @staticmethod
    def get_before_datetime_utc(before):
        return (datetime.datetime.now(tz=timezone.utc) - datetime.timedelta(minutes=before)).strftime(
    ...
    def get_convert_datetime(value, tz_from, tz_to):
        ...
        return datetime.datetime.strftime(display_dt, settings.APP_CONSTANT_DISPLAY_DATETIME_FORMAT)
    ...
    def get_convert_datetime_other(value, tz_from, tz_to):
        ...
        utc_dt = pytz.timezone(tz_from).localize(value)
        display_dt = utc_dt.astimezone(pytz.timezone(tz_to))
        return datetime.datetime.strftime(display_dt, settings.APP_CONSTANT_DISPLAY_DATETIME_FORMAT_OTHER)
