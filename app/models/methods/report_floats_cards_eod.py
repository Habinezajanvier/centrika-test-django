...
class Report_Floats_Cards_EOD():
    @staticmethod
    def get_data_by_values(request, operator, filter_form, filter_date, filter_agent, filter_company=-1, filter_status=-1, card=False, refund=False, balance = False):
        ...
        if not balance:
            if card:
                if refund:
                    objects = objects.filter(
                        Q(agent_float_action=Agent_Floats_Cards.TEXT_ACTION_CARD_TOPUP_REFUND))
                else:
                    objects = objects.filter(
                        Q(agent_float_action=Agent_Floats_Cards.TEXT_ACTION_CARD_TOPUP))
            else:
                if refund:
                    objects = objects.filter(
                        Q(agent_float_action=Agent_Floats_Cards.TEXT_ACTION_DEFAULT_REFUND) |
                        Q(agent_float_action=Agent_Floats_Cards.TEXT_ACTION_TRANSFER_REFUND)
                    )
                else:
                    objects = objects.filter(
                        Q(agent_float_action=Agent_Floats_Cards.TEXT_ACTION_DEFAULT) |
                        Q(agent_float_action=Agent_Floats_Cards.TEXT_ACTION_TRANSFER)
                    )
        ...
        if balance:
            ...
            if filter_status == Agent_Floats_Cards.STATUS_APPROVED and card == False:
    ...
    @staticmethod
    def get_data(request, operator, filter_form, filter_status=-1, card=False, refund=False, balance = False):
