...
class Methods_Agent_Floats_Tickets():
    ...
    def form_view(cls, request, operator, model, api=False):
        if api:
            ...
            return {
                ...
                'ticket_id': model.agent_float_ticket_id,
    ...
    def add(cls, request, operator, agent, data, model=None):
        ...
        try:
            ...
            model.agent_float_status = Agent_Floats_Tickets.STATUS_PENDING
            model.agent_float_bank_reference = ''
            model.save()
        except Exception as e:
            return True, str(e), model
    ...
    @classmethod
    def remove(cls, request, operator, agent, data, model=None, card=None, company=None, number=''):
        data = json.dumps(data)
        data = json.loads(data)
        ...
        if agent.agent_balance < data['amount']:
            return True, 'Insufficient balance.', model

        model.agent_float_agent_id = agent.agent_id
        model.agent_float_agent_name = agent.agent_name
        model.agent_float_type = Agent_Floats_Tickets.TYPE_REMOVE
        model.agent_float_bill = None
        model.agent_float_amount = data['amount']
        model.agent_float_old_balance = agent.agent_balance
        model.agent_float_new_balance = agent.agent_balance - data['amount']
        model.agent_float_reason = data['reason']
        model.agent_float_action = data['action']
        ...
        model.agent_float_status = Agent_Floats_Tickets.STATUS_APPROVED
        model.agent_float_bank_reference = ''
        model.save()
        agent.agent_balance = model.agent_float_new_balance
        agent.save()
        return False, 'Success', model
    ...
    def update_status(cls, request, operator, agent, model, status, action=None, data=None, system_agent=None, sub_agent=None):
        ...
        if sub_agent is not None:
            if status == Agent_Floats_Tickets.STATUS_APPROVED:
                model.agent_float_old_balance = sub_agent.agent_balance
                model.agent_float_new_balance = sub_agent.agent_balance + model.agent_float_amount
                model.save()
                sub_agent.agent_balance = model.agent_float_new_balance
                sub_agent.save()
            if status == Agent_Floats_Tickets.STATUS_DENIED:
                model.save()
        else:
            if status == Agent_Floats_Tickets.STATUS_APPROVED:
                model.agent_float_old_balance = agent.agent_balance
                model.agent_float_new_balance = agent.agent_balance + model.agent_float_amount
                model.save()
                agent.agent_balance = model.agent_float_new_balance
                agent.save()
            if status == Agent_Floats_Tickets.STATUS_DENIED:
    ...
    def get_agent_floats_history_pending(cls, request, operator, agent, date):
        if agent.agent_supervisor == 0:
            objects = Agent_Floats_Tickets.objects.filter(
                Q(agent_float_status=Agent_Floats_Tickets.STATUS_PENDING)
            )
        else:
            objects = Agent_Floats_Tickets.objects.filter(
        ...
        for float_transaction in float_transactions:
            list_floats_transactions_by_agent.append(
                Methods_Agent_Floats_Tickets.form_view(
    ...
    def get_agent_floats_history_approved(cls, request, operator, agent, date):
        if agent.agent_supervisor == 0:
            objects = Agent_Floats_Tickets.objects.filter(
                Q(agent_float_status=Agent_Floats_Tickets.STATUS_APPROVED)
            )
        else:
            objects = Agent_Floats_Tickets.objects.filter(
                Q(agent_float_agent_id=agent.agent_id) &
    ...
    def get_agent_floats_history_denied(cls, request, operator, agent, date):
        if agent.agent_supervisor == 0:
            objects = Agent_Floats_Tickets.objects.filter(
                Q(agent_float_status=Agent_Floats_Tickets.STATUS_DENIED)
            )
        else:
            objects = Agent_Floats_Tickets.objects.filter(
                Q(agent_float_agent_id=agent.agent_id) &
    ...
    @classmethod
    def get_agent_floats_history(cls, request, operator, agent, date, number, float_add, float_add_refund, float_transfer, float_transfer_refund, float_topup, float_topup_refund, status_pending, status_approved, status_declined):
        if agent.agent_supervisor == 0:
        ...
        if number != '':
            objects = objects.filter(
                Q(agent_float_number=number)
            )
        float_actions = []
        if float_add:
            float_actions.append(Agent_Floats_Tickets.TEXT_ACTION_DEFAULT)
        if float_add_refund:
            float_actions.append(Agent_Floats_Tickets.TEXT_ACTION_DEFAULT_REFUND)
        if float_transfer:
            float_actions.append(Agent_Floats_Tickets.TEXT_ACTION_TRANSFER)
        if float_transfer_refund:
            float_actions.append(Agent_Floats_Tickets.TEXT_ACTION_TRANSFER_REFUND)
        if float_topup:
            float_actions.append(Agent_Floats_Tickets.TEXT_ACTION_TOPUP)
        if float_topup_refund:
            float_actions.append(Agent_Floats_Tickets.TEXT_ACTION_TOPUP_REFUND)
        objects = objects.filter(
                Q(agent_float_action__in=float_actions))
        float_status = []
        if status_pending:
            float_status.append(Agent_Floats_Tickets.STATUS_PENDING)
        if status_approved:
            float_status.append(Agent_Floats_Tickets.STATUS_APPROVED)
        if status_declined:
            float_status.append(Agent_Floats_Tickets.STATUS_DENIED)
        objects = objects.filter(
